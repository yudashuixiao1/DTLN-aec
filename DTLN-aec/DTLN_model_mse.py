...
class DTLN_model():
    ...
    def __init__(self):
        ...
        self.cost_function = keras.losses.mean_squared_error #self.snr_cost_mse
        # empty property for the model
        self.model = []
        # defining default parameters
        self.fs = 16000
        self.batchsize = 32
        self.len_samples = 10 #4
        self.activation = 'sigmoid'
        self.numUnits = 512 #128
        self.numLayer = 2
        self.blockLen = 512
    ...
    #     # returning the loss
    #     return loss
    # def snr_cost_mse(s_estimate, s_true):

    #     print('s_estimate', np.shape(s_estimate))
    #     # s_estimate = tf.expand_dims(s_estimate, axis=1)
    #     # s_true = tf.expand_dims(s_true, axis=1)
    #     s_estimate_mag = tf.math.abs(tf.signal.rfft(s_estimate))
    #     s_true_mag = tf.math.abs(tf.signal.rfft(s_true))
    #     print('shape of s_true_mag', s_true_mag)
    #     # if norm_stft:
    #     #     s_estimate_mag = InstantLayerNormalization()(tf.math.log(s_estimate_mag + 1e-7))
    #     #     s_true_mag = InstantLayerNormalization()(tf.math.log(s_true_mag + 1e-7))
    #     # else:
    #     #     s_estimate_mag = s_estimate_mag
    #     #     s_true_mag = s_true_mag
    #     loss = tf.reduce_mean(tf.math.square(s_true_mag - s_estimate_mag ))+1e-7
    #     return loss
    ...
    def lossWrapper():
        ...
        def lossFunction(y_true,y_pred):
            y_estimate_mag = tf.math.abs(tf.signal.rfft(y_pred))
            y_true_mag = tf.math.abs(tf.signal.rfft(y_true))
            # calculating loss and squeezing single dimensions away
            loss = tf.squeeze(keras.losses.mean_squared_error(y_estimate_mag,y_true_mag))
    ...
    def train_model(self, runName, path_to_train_nearend_signal, path_to_train_farend_signal, path_to_train_nearend_speech, \
        ...
        generator_input = audio_generator(path_to_train_nearend_signal, 
                                          path_to_train_farend_signal,
                                          path_to_train_nearend_speech,
                                          len_in_samples, 
        ...
        generator_val = audio_generator(path_to_val_nearend_signal, 
                                        path_to_val_farend_signal,
                                        path_to_val_nearend_speech,
