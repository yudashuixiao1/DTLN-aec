...
class DTLN_model():
    ...
    def __init__(self):
        ...
        win = np.sin(np.arange(.5,self.blockLen-.5+1)/self.blockLen*np.pi)
    ...
    def spectrum_loss(self,y_pred, y_true):
        ...
        spectrum MSE loss 
        '''
        # enh_real, enh_imag = self.tftLayer_s(y_pred)
        # enh_mag = tf.sqrt(enh_real**2 + enh_imag**2 + 1e-8)
        
        # true_real,true_imag = self.tftLayer_s(y_true)
        ...
        enh_mag, enh_angle = self.tftLayer(y_pred)
        true_mag,true_angle = self.tftLayer(y_true)
        loss_mag = tf.reduce_mean((enh_mag - true_mag)**2,) 
        
        return  loss_mag # loss_real + loss_imag + loss_mag
    

    def spectrum_loss_SD(self,s_hat, s, c = 0.3, Lam = 0.1):
        # The complex compressed spectrum MSE loss
        # s = tf.truediv(s,self.batch_gain + 1e-9)
        # s_hat= tf.truediv(s_hat,self.batch_gain + 1e-9)

        true_real,true_imag = self.tftLayer_s(s)
        hat_real,hat_imag = self.tftLayer_s(s_hat)
    ...
    def lossWrapper(self):
        ...
        def lossFunction(y_true,y_pred):
            ...
            mag_loss = self.spectrum_loss_SD(y_pred, y_true)
            # calculate mean over batches
            loss = tf.reduce_mean(loss)
            # return the loss
            return loss + 10*mag_loss
    ...
    def build_DTLN_model_win(self, norm_stft=False):
        ...
        farend_frames = self.win * farend_frames
        nearend_frames = self.win * nearend_frames
        # calculate STFT
        farend_mag,farend_angle = Lambda(self.tftLayer)(farend_frames)
        nearend_mag,nearend_angle = Lambda(self.tftLayer)(nearend_frames)
        ...
        # transform frames back to time domain
        estimated_frames_1 = Lambda(self.ifftLayer)([estimated_mag,nearend_angle])
        #estimated_frames_1 = estimated_frames_1  * self.win
        ...
        # decode the frames back to time domain
        decoded_frames = Conv1D(self.blockLen, 1, padding='causal',use_bias=False)(estimated)
        decoded_frames = decoded_frames * self.win
        # create waveform with overlap and add procedure
        estimated_sig = Lambda(self.overlapAddLayer)(decoded_frames)
    ...
    def train_model(self, runName, path_to_train_nearend_signal, path_to_train_farend_signal, path_to_train_nearend_speech, \
        ...
        checkpointer = ModelCheckpoint(savePath+"epoch-{epoch:02d}-val_loss-{val_loss:.2f}_"+runName +'.h5',
                                       monitor='val_loss',
                                       verbose=1,
                                       save_best_only=True,
